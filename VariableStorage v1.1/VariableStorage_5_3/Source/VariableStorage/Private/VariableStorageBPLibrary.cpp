...
DEFINE_FUNCTION(UVariableStorageBPLibrary::execSetDataToVariableStorage)
{
	...
	if (Data)
	{
		...
		else if (CastField<FSoftObjectProperty>(Data))  //Soft Object
		{
			TypeVariable.Append("SoftObject");
			Variable = FSoftObjectProperty::GetPropertyValue(ValueData).ToString();
		}
		else if (CastField<FSoftClassProperty>(Data))  //Soft Class
		{
			TypeVariable.Append("SoftClass");
			Variable = FSoftClassProperty::GetPropertyValue(ValueData).ToString();
		}
		else if (const FStructProperty* StructData = CastField<FStructProperty>(Data))
		{
			...
			else if (StructData->Struct->GetFName() == FName("Timespan"))  //Timespan
			{
				TypeVariable.Append("Timespan");
				FTimespan Value = *reinterpret_cast<FTimespan*>(ValueData);
				Variable = Value.ToString();
			}
		}
	...
	if (Data)
	{
		...
		else if (InnerProp)
		{
			...
			for (int32 i = 0; i < ArrayHelper.Num(); ++i)
			{
				...
				else if (CastField<FSoftObjectProperty>(InnerProp))  //Soft Object
				{
					if (!HasDoneIt)
					{
						TypeVariable.Append("SoftObject");
					...
					const FSoftObjectProperty* VariableProp = CastField<FSoftObjectProperty>(InnerProp);
					ArrayPart = VariableProp->GetPropertyValue(ElementAddress).ToString();
				}
				else if (CastField<FSoftClassProperty>(InnerProp))  //Soft Class
				{
					if (!HasDoneIt)
					{
						TypeVariable.Append("SoftClass");
					...
					const FSoftClassProperty* VariableProp = CastField<FSoftClassProperty>(InnerProp);
					ArrayPart = VariableProp->GetPropertyValue(ElementAddress).ToString();
					}
				else if (const FStructProperty* StructData = CastField<FStructProperty>(InnerProp))
				{
					...
					else if (StructData->Struct->GetFName() == FName("Timespan"))  //Timespan
					{
						...
						FTimespan Value = *reinterpret_cast<FTimespan*>(ElementAddress);
						Variable = Value.ToString();
					}
				}
				else if (!NonWorkingType)
		...
		else if (VariableStorage.VariableData.Contains(NameData))
		{
			...
			if (Variable.Contains(TypeSeparator))
			{
				...
				if (!TypeVariable.Contains("|Array"))
				{
					...
					{
						if (TypeVariable == "Double" || TypeVariable == "Float")
						{
					...
					{
						if (TypeVariable == "SoftObject")
						{
							FSoftObjectPtr SoftObject(Variable);
							CastField<FSoftObjectProperty>(Data)->SetPropertyValue(ValueData, SoftObject);
						}
						else { NonWorkingType = true; }
					}
					else if (CastField<FSoftClassProperty>(Data))  //Soft Class
					{
						if (TypeVariable == "SoftClass")
						{
							FSoftObjectPtr SoftObject(Variable);
							CastField<FSoftClassProperty>(Data)->SetPropertyValue(ValueData, SoftObject);
						}
						else { NonWorkingType = true; }
					}
					else if (const FStructProperty* StructData = CastField<FStructProperty>(Data))
					{
						...
						else if (StructData->Struct->GetFName() == FName("Timespan"))  //Timespan
						{
							if (TypeVariable == "Timespan")
							{
								...
								*reinterpret_cast<FTimespan*>(ValueData) = Value;
							}
							else { NonWorkingType = true; }
						}
					}
		...
		else if (VariableStorage.VariableData.Contains(NameData))
		{
			...
			if (Variable.Contains(TypeSeparator))
			{
				...
				if (TypeVariable.Contains("|Array"))
				{
					...
					while (!Variable.IsEmpty() && NonWorkingType == false)
					{
						if (CastField<FBoolProperty>(InnerProp))  //Bool
						{
							if (TypeVariable == "Bool")
						...
						else if (CastField<FSoftObjectProperty>(InnerProp))  //Soft Object
						{
							if (TypeVariable == "SoftObject")
							{
								...
								if (Variable.Contains(VariableSeparator))
								{
									...
									FSoftObjectPtr SoftObject(Variable);
									const FSoftObjectProperty* VariableProp = CastField<FSoftObjectProperty>(InnerProp);
									VariableProp->SetPropertyValue(ElementAddress, SoftObject);
									Variable = RemainingPart;
									ArrayIndex++;
								}
								else
							...
							else if (StructData->Struct->GetFName() == FName("Timespan"))  //Timespan
							{
								if (TypeVariable == "Timespan")
								{
									...
									if (Variable.Contains(VariableSeparator))
									{
										Variable.Split(VariableSeparator, &Variable, &RemainingPart);
										Value.Parse(Variable, Value);
										*reinterpret_cast<FTimespan*>(ElementAddress) = Value;
										Variable = RemainingPart;
										ArrayIndex++;
									}
									else
